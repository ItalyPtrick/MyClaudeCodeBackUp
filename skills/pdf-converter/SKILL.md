---
name: pdf-converter
description: "PDF converter powered by MinerU for heavy-duty document extraction. Use this skill when the user has a complex or large PDF, scanned document, OCR-heavy file, difficult layout, formula/table preservation need, or wants conversion to Markdown, HTML, LaTeX, DOCX, or other structured output formats. Prefer the lighter `pdf` skill for simple reading, summarization, and ordinary PDF Q&A tasks."
---
...
Use `-o` to save to a file when the user wants persistent output (conversion, batch processing). This skill is for heavier extraction/conversion work, not ordinary PDF reading or lightweight Q&A.

For example:
- "帮我把这个复杂扫描版 PDF 转成 markdown" → use `-o` to save to file, done
- "提取这份复杂论文里的表格和公式" → use `-o` to save, then read the file and pull out the tables/formulas
- "把这个 OCR 很差的扫描件转成可编辑文本" → prefer OCR-oriented extraction
- "把 PDF 里的参考文献整理出来，而且版式很乱" → stdout or `-o`, then parse the references section

If the task is just reading a normal PDF, answering simple questions about its contents, or writing a lightweight summary, prefer the `pdf` skill instead.

## Two Extraction Modes

### flash-extract — Fast extraction, no auth

Best for quick preprocessing of complex documents when you still need MinerU-level extraction but do not need the heavier authenticated mode. No API key, no setup.

```bash
mineru-open-api flash-extract report.pdf                               # extract quickly for complex-document processing
mineru-open-api flash-extract report.pdf -o ./output/                  # save extracted markdown to file
...
Use flash-extract only for lighter heavy-duty extraction cases within this skill. If the overall task is just normal PDF reading or simple summarization, prefer the `pdf` skill instead of this one.
...
| Situation | Mode |
|---|---|
| Complex PDF but markdown output is enough | flash-extract |
...
| Simple reading / summary / ordinary Q&A | Use `pdf`, not this skill |
